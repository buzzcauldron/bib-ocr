...
def test_footnote_scan_falls_back_to_all_pages_when_density_hot_list_empty(monkeypatch, tiny_pdf):
    """Empty target_pages() must not skip footnote OCR — treat like None (scan all pages)."""
    import bib_ocr.stages.doi_scan as doi_scan
    import bib_ocr.stages.link_crawl as link_crawl
    import bib_ocr.stages.ref_section as ref_section
    ...
    footnote_calls: list[list[int] | None] = []

    def capture_footnote(pdf_path, target_page_indices=None, **_kw):
        footnote_calls.append(target_page_indices)
        return []

    monkeypatch.setattr(footnote_scan, "extract", capture_footnote)
